...
function drawTable(endpoint) {
	...
	d3.json(endpoint).then(function(asmr_data) {
        ...
        console.log(asmr_data)
				...
				headers = headers.slice(5,6).concat(headers.slice(17,18)).concat(headers.slice(12,13)).concat(headers.slice(19,20))
		...
		var content = tbody.selectAll('tr').data(asmr_data).enter()
			...
			.html((data, i) => (`
			  <td class="col_0 row_${i + 1}">${data.channel_name}</td><td class="col_1 row_${i + 1}">${data.uploads}</td>
				...
				<td class="col_4 row_${i + 1}">${data.channel_age}</td><td class="col_5 row_${i + 1}">${data.upload_frequency}</td>
				<td class="col_6 row_${i + 1}">${data.avg_views_video}</td><td class="col_7 row_${i + 1}">${data.avg_views_subscriber}</td>
